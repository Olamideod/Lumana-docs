...
3. Add a user: Select **Add user**. Enter a username and strong password, set **Role** to **Administrator** (or the minimum role your security team approves). Note that lower roles may block some Lumana features.
...
* **Name the profile before you finish**: Enter the final **profile name** in the Axis editor, save, and reuse that exact string in Lumana’s RTSP or profile field. If you rename the profile in Axis later, then update Lumana to match.
